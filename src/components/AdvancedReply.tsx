...
import { Plus, Download, Upload, Trash2, Code, Edit } from 'lucide-react';
...
export const AdvancedReply: React.FC = () => {
  const { advancedRules, loading, fetchRules, addAdvancedRule, updateAdvancedRule, deleteAdvancedRule } = useRulesStore();
  ...
  const handleSaveRule = async (rule: Rule) => {
    try {
      setSaving(true);
      if (rule.id === 'new') {
        await addAdvancedRule(rule);
      } else {
        await updateAdvancedRule(rule);
      }
      setEditingRule(null);
    } finally {
      setSaving(false);
  ...
  const exportRules = () => {
    const csv = Papa.unparse(advancedRules.map(rule => ({
  ...
  if (loading && advancedRules.length === 0) {
    return (
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">HTML</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {advancedRules.map((rule) => (
              <tr key={rule.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <div className={`text-sm ${rule.isHtml ? 'text-purple-600' : 'text-gray-500'}`}>
                    {rule.isHtml ? 'Enabled' : 'Disabled'}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  ...
                  <button
                    onClick={() => deleteAdvancedRule(rule.id)}
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    onClick={() => setEditingRule({ ...editingRule, isHtml: !editingRule.isHtml })}
                    className={`flex items-center gap-1 px-2 py-1 rounded ${
                      editingRule.isHtml ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-700'
                    }`}
                  >
                    <Code className="w-4 h-4" />
                    HTML {editingRule.isHtml ? 'Enabled' : 'Disabled'}
                  </button>
                </div>
                <textarea
                  ...
                  placeholder={editingRule.isHtml ? '<p>Enter HTML response here...</p>' : 'Enter response here...'}
                />
              </div>
            </div>
